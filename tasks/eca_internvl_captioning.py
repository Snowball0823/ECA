...
"""
"""ECA continual captioning task for InternVL alignment models."""
...
@registry.register_task("eca_internvl_captioning")
class ECAInternVLCaptionTask(ECAQCaptionTask):
    """Caption task variant tailored for the InternVL-based ECA model."""
        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            if dictionary_loader is not None:
                ...
                if replay_feature is not None and replay_feature.dim() == 2 and patch_len:
                    replay_feature = replay_feature.view(-1, patch_len, replay_feature.size(-1))
                ...
                if self.local_ewc:
                    ...
                    for name, param in unwrap_model.trainable_adapter_parameters:
                        if name in adapter_fisher and name in self.ewc_paramters:
                            ...
                            ewc_loss += (
                                torch.sum(adapter_fisher[name] * (param - ref_param).pow(2)) / 2
                            )
                    loss += ewc_loss
                    loss_dict["loss"] = loss
    ...
    def before_training(
        ...
        **kwargs,
        ...
        lora_visual = False
        unwrap_model = unwrap_dist_model(model)
        dictionary_dataset = None
        patch_len = getattr(self, "_dict_patch_len", None)
        if self.use_dictionary and patch_len is None:
            vision = unwrap_model.vision_tower
            if vision is None:
                raise RuntimeError("Vision tower is not initialized; cannot determine dictionary replay patch length.")
            patch_len = getattr(unwrap_model, "vision_patch_len", None)
            if patch_len is None:
                patch_len = getattr(vision, "patch_len", None)
            if patch_len is None:
                patch_len = getattr(vision, "num_patches", None)
            if patch_len is None:
                raise RuntimeError("Vision tower does not expose a fixed patch length for dictionary replay.")
        ...
        if cur_task > 0:
            if self.use_dictionary:
                ...
                batch_sizes = self.train_batch_size//2
                if patch_len:
        ...
        if self.lora_visual_first and cur_task == 0:
            lora_visual = True
        if self.expand_pa and cur_task == 6:
            expand = True
        if cur_task > 6 and self.expand_pa:
            logging.info("Start calculating Initial FIM for Task {}".format(cur_task+1))
            fisher_start = time.time()
            unwrap_model.train()
            self.init_fisher = self.update_adapters_fisher(cur_task, unwrap_model, dataloader, optimizer, scaler=scaler, cuda_enabled=cuda_enabled)
    ...
    def update_adapters_fisher(
        ...
        for idx in iterator:
            samples = next(data_loader)
            samples = prepare_sample(samples, cuda_enabled=cuda_enabled)
            fisher = self._calculate_adapters_fisher_step(idx, unwrap_model, samples, fisher, scaler)
    ...
    def final(self, cur_task, model, dataloader, optimizer, scaler=None, cuda_enabled=True):
        ...
        if self.expand_pa or self.global_ewc:
            logging.info(f"Start calculating Final FIM for Task {cur_task+1}")
            fisher_start = time.time()
            unwrap_model.train()
        ...
        if self.use_dictionary:
            unwrap_model.eval()
            logging.info(f"Updating Feature Dictionary for Task {cur_task+1}")
    ...
    def _report_metrics(self, eval_result_file, split_name):
        return super()._report_metrics(eval_result_file, split_name)
