...
"""
"""ECA continual VQA task for InternVL alignment models."""
...
@registry.register_task("eca_internvl_vqa")
class ECAInternVLVQATask(ECAQVQATask):
    """VQA task variant tailored for the InternVL-based ECA model."""
    ...
    def before_training(
        ...
        **kwargs,
        ...
        if self.use_dictionary and patch_len is None:
            ...
            if vision is None:
                raise RuntimeError(
                    "Vision tower is not initialized; cannot determine dictionary replay patch length."
                )
            ...
            if patch_len is None:
                raise RuntimeError(
                    "Vision tower does not expose a fixed patch length for dictionary replay."
                )
