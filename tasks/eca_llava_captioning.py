...
"""
"""ECA continual captioning task for LLaVA alignment models."""
...
@registry.register_task("eca_llava_captioning")
class ECALlavaCaptionTask(ECAQCaptionTask):
    """Caption task variant tailored for the LLaVA-based ECA model."""
    ...
    def before_training(
        ...
        **kwargs,
        ...
        if self.use_dictionary and patch_len is None:
            ...
            if vision is None:
                raise RuntimeError("Vision tower is not initialized; cannot determine patch number.")
            patch_len = getattr(vision, "num_patches", None)
            if patch_len is None:
                raise RuntimeError("Vision tower does not expose num_patches; please ensure LLaVA v0 vision tower is used.")
