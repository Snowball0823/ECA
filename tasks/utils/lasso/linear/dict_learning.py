...
def dict_learning(X, n_components, alpha=1.0, constrained=True, persist=False,
    ...
    with tqdm(total=steps, disable=not progbar) as progress_bar:
        for i in range(steps):
            ...
            Z = sparse_encode(X, weight, alpha=alpha, z0=Z0, **solver_kwargs)
...
def update_dict(dictionary, X, Z, random_seed=None, positive=False,
    ...
    # R = X - torch.matmul(Z, dictionary.T)  # (n_samples, n_features)
    a = Z.T@Z
    A = Z.T@(Z@dictionary.T)
    B = Z.T@X
    r = 0
    eps = torch.tensor(eps).to(A.device)
    for k in range(n_components):
        # Update k'th atom
        r += torch.outer(Z[:, k], dictionary[:, k])
        # R += torch.outer(Z[:, k], dictionary[:, k])
        # dictionary[:, k] = B[k] - A[k] + Z[:, k]@r
        ...
        # start = time.time()
        # a = atom_norm < eps
        # print('Norm calculation time: {}'.format(time.time() - start))
        # input()
        start = time.time()
        # if atom_norm < eps:
        if a[k, k] < 1e-6:
