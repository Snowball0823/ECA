...
from .coda_blip2 import Blip2Base, disabled_train
from .utils import freeze_parameters, orthogonal_svd_init, tensor_prompt
from .zoo import CodaPrompt, L2P, DualPrompt


@registry.register_model("coda_blip2_opt")
class CODABlip2OPT(Blip2Base):
    """
    BLIP2 OPT model.
    ...
    def __init__(
        ...
        if self.prompt_type not in {"coda", "dual"}:
            raise ValueError(f"Unsupported prompt_type: {prompt_type}. Use 'coda' or 'dual'.")
        self.n_tasks = int(n_tasks)
        if prompt_param is None:
            prompt_param = [300, 8, 0.0] if self.prompt_type == "coda" else [10, 500, 500]
        self.prompt_param = list(prompt_param)
    ...
    def adapter_structure(self):
        structure = {'lora': self.lora_dict, 'adapter': self.parallel_adapters_dict, 
                                  ...
                                  'q_embedding_freeze': self.qformer_embedding_freezed, 'moq_num': self.moq_num,
                                  'prompt_type': self.prompt_type, 'n_tasks': self.n_tasks,
                                  'prompt_param': list(self.prompt_param)}
        return structure
    ...
    def get_optimizer_params(self, weight_decay, lr_scale=1):
        ...
        for name, param in self.named_parameters():
            ...
            elif name.startswith("prefix_prompt."):
                grouped_named_params["prompt"].append((name, param))
            elif self.lora_prefix and self.lora_prefix in name:
            ...
            elif "Qformer.bert.embeddings" in name:
                grouped_named_params["qformer_embedding"].append((name, param))
            else:
        ...
        summarize("prompt", grouped_named_params["prompt"])
        summarize("vision_lora", grouped_named_params["vision_lora"])
        summarize("moq", grouped_named_params["moq"])
        summarize("qformer_embedding", grouped_named_params["qformer_embedding"])

        other_entries = grouped_named_params["other"]
        if other_entries:
            warning_msg = (
                "Unexpected trainable params found outside projector/prompt/vision_lora/MoQ/QFormer embeddings. "
    ...
    def before_training(self, lora_visual=False, **kwargs):
        ...
        if self.prefix_prompt is None:
            self.init_coda_prompt()
        else:
            self.prefix_prompt.process_task_count()
    ...
    def rebuild_from_config(self, adapter_structure: dict, moq_old_kv: dict, **kwargs):
        ...
        saved_prompt_type = adapter_structure.get('prompt_type')
        if saved_prompt_type is not None and saved_prompt_type != self.prompt_type:
            raise ValueError(
                f"Checkpoint prompt_type={saved_prompt_type} does not match config prompt_type={self.prompt_type}."
            )
        saved_n_tasks = adapter_structure.get('n_tasks')
        if saved_n_tasks is not None and int(saved_n_tasks) != self.n_tasks:
            raise ValueError(
                f"Checkpoint n_tasks={saved_n_tasks} does not match config n_tasks={self.n_tasks}."
            )
        saved_prompt_param = adapter_structure.get('prompt_param')
        if saved_prompt_param is not None and list(saved_prompt_param) != list(self.prompt_param):
            raise ValueError(
                f"Checkpoint prompt_param={saved_prompt_param} does not match config prompt_param={self.prompt_param}."
            )

        if self.prefix_prompt is None:
            self.init_coda_prompt()
            assert 'task_num' in kwargs, 'Resume for Prompt Based Model need parameter: "task_unm".'
            self.prefix_prompt.task_count = kwargs['task_num']


    def init_coda_prompt(self):
        freeze_parameters(self, ['query_tokens'])
        for param in self.Qformer.bert.parameters():
            param.requires_grad = False
        prompt_kwargs = {
        ...
        if self.prompt_type == "coda":
            self.prefix_prompt = CodaPrompt(self.Qformer.bert.config.hidden_size, **prompt_kwargs)
        elif self.prompt_type == "dual":
            self.prefix_prompt = DualPrompt(self.Qformer.bert.config.hidden_size, **prompt_kwargs)
        else:
            raise ValueError(f"Unsupported prompt_type: {self.prompt_type}. Use 'coda' or 'dual'.")
    ...
    def forward(self, samples: dict):
        ...
        self.prefix_prompt.batch_reset(image_embeds[:,0,:])
        # Q-Former part
        ...
        ortho_loss = 0
        key_task_loss = 0
        loss = loss + self.prefix_prompt.o_loss
        if self.mix_query and self.ortho:
            ...
            query_ortho_loss = torch.norm(query_gram_matrix, p='fro')
            
            ortho_loss = key_ortho_loss+query_ortho_loss

            loss = loss + self.ortho_weight*(ortho_loss+key_task_loss)

        return {"loss": loss, 'output loss': outputs.loss, 'Prompt loss': self.prefix_prompt.o_loss, 'DR loss': mse_loss, 'MoQ loss': (ortho_loss+key_task_loss)}
    ...
    def mixture_of_query(self, image_embeds: torch.Tensor, query_tokens: nn.Parameter, old_only=False):
        if hasattr(self, 'current_keys') and hasattr(self, 'current_queries'):
            ...
            # Step 1: Aggregate image tokens into one image-level feature.
            ...
            # Step 2: Compute task attention scores from cosine similarity with task keys.
            img_norm = F.normalize(img_patches, p=2, dim=-1)
            ...
            # Step 3: Apply task attention scores to task queries to get delta query.
    ...
    def from_config(cls, cfg):
        ...
        prompt_type = cfg.get("prompt_type", "coda")
        n_tasks = cfg.get("n_tasks", 10)
        prompt_param = cfg.get("prompt_param", None)
