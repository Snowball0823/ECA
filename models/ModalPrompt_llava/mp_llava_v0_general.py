...
@registry.register_model("mp_llava_v0_general")
class ModalPromptLlavaV0General(SoftpromptLlavaBase):
    """ModalPrompt baseline adapted for the LLaVA-v0 backbone.
    ...
    def __init__(
        ...
        kd_prompt: str = "",
        use_modal_prompt: bool = False,
        mp_prefix_len: int = 10,
        mp_transfer_num: int = 3,
        mp_lam: float = 0.5,
        mp_loss_weight: float = 1.0,
        mp_clip_model_name: str = "openai/clip-vit-large-patch14-336",
        ...
        **loader_kwargs,
        ...
        if freeze_vit:
            self.freeze_vision_modules(disable_training=True)
        ...
        # ModalPrompt (dynamic prompt tokens + CLIP guidance)
        ...
        self._mp_embed_hook = None
        self._mp_embed_mask = None
        ...
        # Adapter bookkeeping ------------------------------------------------
        self.parallel_adapters_dict: dict[int, List[str]] = {}
        self.adapter_init = False
        self.llm_frozen = False
        leave_out = list(range(alignment_layers, self.llava_model.config.num_hidden_layers))
    ...
    def get_optimizer_params(self, weight_decay, lr_scale=1):
        ...
        param_groups = []
        projector_params = [p for _, p in grouped_named_params["projector"]]
        ...
        other_params = [p for _, p in grouped_named_params["other"]]
            ...
            param_groups.append(
                {
                    "weight_decay": 0.0,
                    "lr_scale": 1,
                    "params": modal_prompt_params,
                }
            )
        if other_params:
            warning_msg = (
                "Unexpected trainable params found outside 'mm projector/LLaMa adapters/CLIP LoRA/MoQ/ModalPrompt'. "
    ...
    # ------------------------------------------------------------------
    # ModalPrompt helpers (dynamic prompt tokens + CLIP guidance)
    # Initialize CLIP encoders for prompt guidance.
    ...
    # Ensure CLIP modules live on the requested device.
    ...
    # Add a new task prompt bank and its projection head.
    def _mp_add_new_task_prompt(self, task_id: int):
        ...
        hidden_dim = self.llava_model.config.hidden_size
        embed_weight = self.llava_model.get_model().embed_tokens.weight
        with torch.no_grad():
    ...
    # Restore prompt tokens and transforms from saved state.
    ...
    # Activate only the current task prompt parameters.
    def _mp_set_current_task(self, cur_task: int):
        ...
        # keep LLM embeddings frozen to avoid FP16 trainable params
        embed_weight = self.llava_model.get_model().embed_tokens.weight
    ...
    # Encode images with CLIP for prompt selection.
    def _mp_encode_image(self, images: Tensor):
        self._mp_ensure_clip_device(images.device)
        pixel_values = images.to(dtype=self.mp_clip_vision.dtype, device=self.mp_clip_vision.device)
        with torch.no_grad():
            outputs = self.mp_clip_vision(pixel_values=pixel_values)
        return outputs.image_embeds

    # Encode text with CLIP for prompt selection.
    ...
    # Compute prompt prototypes in CLIP feature space.
    ...
    # Select top-k prompt indices based on dual-modality similarity.
    def _mp_select_prompt_ids(
        self,
        proto_embeddings: Tensor,
        image_feats: Tensor,
        text_feats: Tensor,
        training: bool,
    ):
        ...
        # Build prompt embeddings for the batch and compute alignment loss.
    def _mp_prepare_prompts(self, images: Tensor, human_messages: List[str], training: bool):
    ...
    # Insert prompt tokens after BOS in the prefix sequence.
    def _mp_insert_prompt_tokens(
        self,
        prefix_ids: Tensor,
        prefix_mask: Tensor,
        ...
        if prompt_ids is None or prompt_ids.numel() == 0:
            return prefix_ids, prefix_mask, prefix_lengths
        prompt_ids = prompt_ids.to(prefix_ids.device)
        prompt_mask = torch.ones_like(prompt_ids, dtype=prefix_mask.dtype, device=prefix_mask.device)
        prefix_ids = torch.cat([prefix_ids[:, :1], prompt_ids, prefix_ids[:, 1:]], dim=1)
        prefix_mask = torch.cat([prefix_mask[:, :1], prompt_mask, prefix_mask[:, 1:]], dim=1)
        prefix_lengths = prefix_lengths + prompt_ids.size(1)
        return prefix_ids, prefix_mask, prefix_lengths

    # Replace prompt token embeddings with learned prompt vectors.
    def _mp_apply_prompt_embeddings(self, inputs_embeds: Tensor, input_ids: Tensor):
    ...
    def extract_visual_feature(self, samples):
        embeds = self.encode_image(samples["image"])
    ...
    def before_training(self, expand_adapters: bool = True, lora_visual: bool = False, **kwargs):
        ...
        if not self.llm_frozen:
            llama_core = self.llava_model.get_model()
            if hasattr(llama_core, "freeze_model"):
                llama_core.freeze_model()
            else:
                self._set_requires_grad(llama_core, False)
    ...
    def _build_sequences(self, samples, batch_size):
        ...
        return prefixes, answers, full_sequences, human_messages
    ...
    def forward(self, samples):
        ...
        prefixes, answers, _, human_messages = self._build_sequences(samples, batch_size)
        ...
        if self.use_modal_prompt and self.mp_prompt_embeddings:
            modal_prompt, mp_loss = self._mp_prepare_prompts(
                image, human_messages, training=self.training
            )
        ...
        if modal_prompt is not None:
            modal_prompt = modal_prompt.to(device)
    ...
    def generate(
        ...
        **kwargs,
        ...
        with self.maybe_autocast():
            projector_tokens, prompt_tokens, prefixes, _, _, human_messages = self._forward_internal_for_generation(samples)
            ...
            if self.use_modal_prompt and self.mp_prompt_embeddings:
                modal_prompt, _ = self._mp_prepare_prompts(
                    samples["image"], human_messages, training=False
                )
            ...
            stop_words = ["###"]
            stop_input_ids = input_ids
            stopping_criteria = StoppingCriteriaList(
                [KeywordsStoppingCriteria(stop_words, self.llava_tokenizer, stop_input_ids)]
            )
    ...
    def predict_answers(
        ...
        **kwargs,
        ...
        with self.maybe_autocast():
            projector_tokens, prompt_tokens, prefixes, _, _, human_messages = self._forward_internal_for_generation(samples, prompt_override=prompt)
            device = projector_tokens.device
            input_ids, attention_mask, _ = tokenize_with_image_support(
                self.llava_tokenizer,
                prefixes,
    ...
    def _forward_internal_for_generation(self, samples, prompt_override: Optional[str] = None):
        ...
        prefixes, answers, full_sequences, human_messages = self._build_sequences(samples, batch_size)

        return projector_tokens, prompt_tokens, prefixes, answers, full_sequences, human_messages
    ...
    def rebuild_from_config(self, adapter_structure: dict, moq_old_kv: dict, **kwargs):
        ...
        if adapter_info:
            # TODO: consider more senario for MoQ restoration.
            # Current is only suitable for the saved final ckpt.
...
class ModalPromptModule(nn.Module):
    """CLIP-guided prompt bank for ModalPrompt."""
    ...
    def __init__(
        ...
        self.current_prompt = nn.Parameter(torch.zeros(prompt_len, hidden_dim))
        nn.init.normal_(self.current_prompt, mean=0.0, std=0.02)
        self.register_buffer(
            "prompt_history",
            torch.empty(0, prompt_len, hidden_dim),
            persistent=True,
        )
        ...
        for module in (self.clip_vision, self.clip_text):
            module.requires_grad_(False)
            module.eval()

    def _device(self):
        return self.current_prompt.device

    def reset_current_prompt(self):
        with torch.no_grad():
            nn.init.normal_(self.current_prompt, mean=0.0, std=0.02)

    def commit_current_prompt(self):
        prompt = self.current_prompt.detach().clone().unsqueeze(0)
        if self.prompt_history.numel() == 0:
            history = prompt
        else:
            history = torch.cat([self.prompt_history, prompt], dim=0)
        self.prompt_history = history.to(self._device())

    def history_size(self):
        if self.prompt_history.numel() == 0:
            return 0
        return self.prompt_history.size(0)

    def encode_image(self, images: Tensor):
        pixel_values = images.to(
            dtype=self.clip_vision.dtype, device=self.clip_vision.device
        )
        with torch.no_grad():
    ...
    def encode_text(self, texts: List[str]):
        ...
        tokenized = self.clip_tokenizer(
            ...
            max_length=77,
            return_tensors="pt",
        ).to(self.clip_text.device)
        with torch.no_grad():
            outputs = self.clip_text(**tokenized)
        if outputs.pooler_output is not None:
            return outputs.pooler_output
        return outputs.last_hidden_state[:, 0, :]
    ...
    def get_prototypes(self):
        ...
        pooled = history.mean(dim=1)
        return self.projector(pooled)

    def current_prototype(self):
        pooled = self.current_prompt.mean(dim=0, keepdim=True)
        return self.projector(pooled)

    def compose_query(self, image_feats: Tensor, text_feats: Tensor):
        lam = torch.clamp(torch.tensor(self.lam, device=image_feats.device), 0.0, 1.0)
        while lam.dim() < image_feats.dim():
            lam = lam.unsqueeze(0)
        mixture = lam * image_feats + (1.0 - lam) * text_feats
        return F.normalize(mixture, dim=-1)

    def build_prompt(self, query_embed: Tensor):
        batch = query_embed.size(0)
        base = self.current_prompt.unsqueeze(0).expand(batch, -1, -1)

        history_prompts, scores = self._select_history_prompts(query_embed)
        if history_prompts is None:
            return base

        weights = F.softmax(scores, dim=-1).unsqueeze(-1).unsqueeze(-1)
        blended = (weights * history_prompts).sum(dim=1)
        return base + blended

    def _select_history_prompts(self, query_embed: Tensor):
        history = self.prompt_history
        if history.numel() == 0 or self.top_k <= 1:
            return None, None
        ...
        hist_per_batch = min(self.top_k - 1, prototypes.size(0))
        if hist_per_batch <= 0:
            return None, None

        query = F.normalize(query_embed, dim=-1)
        proto = F.normalize(prototypes, dim=-1)
        sims = torch.matmul(query, proto.t())
        scores, indices = sims.topk(hist_per_batch, dim=-1)

        gathered = []
        for row in indices:
            gathered.append(history[row])
        stacked = torch.stack(gathered, dim=0)
        return stacked, scores

    def compute_alignment_loss(self, image_feats: Tensor, text_feats: Tensor):
        if self.loss_weight <= 0:
            return image_feats.new_zeros(())
        proto = F.normalize(self.current_prototype(), dim=-1)
        img = F.normalize(image_feats, dim=-1)
        txt = F.normalize(text_feats, dim=-1)
        proto = proto.expand_as(img)
        img_loss = 1.0 - F.cosine_similarity(img, proto, dim=-1)
        txt_loss = 1.0 - F.cosine_similarity(txt, proto, dim=-1)
        return (img_loss + txt_loss).mean()

    def modal_state(self):
        return {
            "current_prompt": self.current_prompt.detach().cpu(),
            "prompt_history": self.prompt_history.detach().cpu(),
            "projector": deepcopy(self.projector.state_dict()),
        }

    def load_modal_state(self, state: Dict[str, Tensor]):
        if not state:
            return
        cp = state.get("current_prompt")
        if cp is not None:
            self.current_prompt.data.copy_(cp.to(self.current_prompt.device))
        history = state.get("prompt_history")
        if history is not None:
            self.prompt_history = history.to(self._device())
        projector = state.get("projector")
        if projector is not None:
            missing, unexpected = self.projector.load_state_dict(projector, strict=False)
            if missing:
                logging.warning("ModalPrompt projector missing keys: %s", missing)
            if unexpected:
                logging.warning("ModalPrompt projector unexpected keys: %s", unexpected)

    def state_dict(self, destination=None, prefix="", keep_vars=False):
        state = super().state_dict(destination, prefix, keep_vars)
        clip_keys = [k for k in state if k.startswith(f"{prefix}clip_")]
        for key in clip_keys:
            state.pop(key)
        return state

    def load_state_dict(self, state_dict, strict=True):
        clipped = {k: v for k, v in state_dict.items() if not k.startswith("clip_")}
        super().load_state_dict(clipped, strict=False)
