...
@registry.register_model("pa_llava_v0_general")
class PALlavaV0General(SoftpromptLlavaBase):
    ...
    def __init__(
        ...
        **loader_kwargs,
        ...
        Args:
            ...
            use_ocr_input: Whether OCR side input is appended to the human prompt.
    ...
    def generate(
        ...
        **kwargs,
        ...
        with self.maybe_autocast():
            ...
            stop_words = ["###"]
            # Use the prompt input_ids to align stop offsets with LLaVA's keyword criterion.
    ...
    def teacher_forward(
        self,
        samples,
        use_old_moq: bool = True,
        use_teacher_adapters: bool = True,
        use_grad: bool = False,
    ):
        """
        Lightweight teacher forward used by LwF.

        Args:
            samples: input batch.
            use_old_moq: use only historical MoQ keys/queries when available.
            use_teacher_adapters: switch to historical adapters for teacher logits.
            use_grad: enable gradients for student-style logits.
        """
        llama_core = self.llava_model.get_model()
        prev_model_mode = self.llava_model.training
        prev_core_mode = llama_core.training
        ...
        with no_grad_ctx:
            ...
            prompt_tokens = self.mixture_of_query(image_embeds, old_only=use_old_only) if self.mix_query else None
        ...
        distillable = torch.clamp(answer_lengths-1, min=0)
        kd_mask = torch.zeros(outputs.logits.shape[:2], dtype=torch.bool, device=device)
        if answer_mask.size(1) > 0:
            token_idx = torch.arange(answer_mask.size(1), device=device).unsqueeze(0)
            suffix_mask = token_idx < distillable.unsqueeze(1)
            kd_mask[:, -answer_mask.size(1):] = suffix_mask
