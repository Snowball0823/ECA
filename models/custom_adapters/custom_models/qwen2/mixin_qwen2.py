...
class Qwen2AttentionMixin:
    def init_adapters(self, model_config, adapters_config):
        ...
        # TODO: Consider wrapping self.o_proj if we want to mirror InternVL's
        # upstream LoRA target modules exactly. For the first pass we keep the
        # same coverage as the existing llama/llava adapter path.
        self.prefix_tuning = PrefixTuningLayer("self_prefix", model_config, adapters_config)

        patch_forward(self)


class Qwen2DecoderLayerMixin:
    def init_adapters(self, model_config, adapters_config):
        ...
        # TODO: Consider wrapping self.mlp.gate_proj if we want to mirror
        # InternVL's upstream LoRA target modules exactly.
...
class Qwen2ModelAdapterMixin(EmbeddingAdaptersMixin, InvertibleAdaptersMixin, ModelBaseAdaptersMixin):
