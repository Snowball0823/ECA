...
from ..custom_adapters.utils import (Average, Fuse, freeze_adapter, init_adapter,
                                     print_trainable_parameters)
from ..ECA_Q.blip2 import Blip2Base, disabled_train
from ..ECA_Q.utils import freeze_parameters, orthogonal_svd_init, tensor_prompt


@registry.register_model("lora_blip2_opt")
class LORABlip2OPT(Blip2Base):
    """
    BLIP2 OPT model.
    ...
    def get_optimizer_params(self, weight_decay, lr_scale=1):
        ...
        moe_lora_prefixes = (self.moe_lora_prefix, self.ffn_pa_prefix)
        ...
        for name, param in self.named_parameters():
            ...
            elif any(prefix in name for prefix in moe_lora_prefixes):
                grouped_named_params["moe_lora"].append((name, param))
        ...
        summarize("moe_lora", grouped_named_params["moe_lora"])
        ...
        if other_entries:
            warning_msg = (
                "Unexpected trainable params found outside projector/MoE-LoRA/vision_lora/MoQ. "
    ...
    def init_moe_lora(self):
        ...
        self.Qformer.bert.add_adapter_fusion(moe_lora_names, 'dynamic')
        self.Qformer.bert.active_adapters = [Fuse(*moe_lora_names)]
    ...
    def extract_query_dictionary(self, feature_dictionary: torch.Tensor):
        ...
        query_tokens = self.mixture_of_query(feature_dictionary, self.query_tokens, old_only=True)
        query_output = self.Qformer.bert(
            query_embeds=query_tokens,
